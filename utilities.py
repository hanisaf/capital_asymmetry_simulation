...
class Runner:
    ...
    def run_configurations(SimulationClass, parameters_ranges, runs_per_configuration = 100, auto_seed=True, seeds = None, parallel=False, verbose = True):
        ...
        if parallel:      
            ...
            with concurrent.futures.ProcessPoolExecutor(cpu_count) as executor:
                futures = [executor.submit(Runner.run_configuration, SimulationClass,  conf, conf_number, seeds, verbose) for conf_number, conf in enumerate(configurations)]
            for f in concurrent.futures.as_completed(futures):
                results.extend(f.result())
            
        else:
            for conf_number, conf in enumerate(configurations):
                result = Runner.run_configuration(SimulationClass, conf, conf_number, seeds, verbose)
    ...
    def create_result_table(list_of_simulations, history=True, round_values=True, agg_function='last'):
        data = pd.concat((Runner.extract_history(s) for s in list_of_simulations))
        ...
        if round_values: data = Runner.round_float_values_in_data_frame(data)
        return data
...
if __name__ == '__main__':
    ...
    results = Runner.run_configurations(SimulationClass, params, parallel=False, runs_per_configuration=5)
    data = Runner.create_result_table(results)
    sim = results[0]
    print(data.head())
    print()